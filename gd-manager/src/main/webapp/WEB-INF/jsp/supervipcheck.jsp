...
<body>
	<div class="route_bg">
		<a>用户审核</a>&nbsp;<i class="glyphicon glyphicon-chevron-right"></i><a>&nbsp;高级会员审核</a>
	</div>


	<div>
		<table id="superviptable">
			<thead style="background-color: #003D79; color: #FFF"></thead>
		</table>
	</div>
</body>
<script>
	$("#superviptable").bootstrapTable(
					{
						idField : "id",
						uniqueId : "id",
						pagination : true,
	...
	function accept(id,userId) {
        isAccept = window.confirm("确定同意其成为高级会员?");
        if(isAccept==true){
        	$.ajax({
        	...
        	success:function(data){
        		if(data.status==200){
					$("#superviptable").bootstrapTable('remove', {
						field : 'id',
						values : [ parseInt(id) ]
					});
					alert(data.msg);
	...
	function refuse(id,userId){
		isRefuse = window.confirm("确定拒绝其成为高级会员?");
		if(isRefuse==true){
			$.ajax({
				url : "/vip/updatevipapplication",
				data:{
