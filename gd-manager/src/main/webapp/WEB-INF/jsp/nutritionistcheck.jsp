...
<body>
	<div class="route_bg">
		<a>用户审核</a>&nbsp;<i class="glyphicon glyphicon-chevron-right"></i><a>&nbsp;营养师审核</a>
	</div>


	<div>
		<table id="nutritionisttable">
			<thead style="background-color: #003D79; color: #FFF"></thead>
		</table>
	</div>
</body>
<script>
	$("#nutritionisttable").bootstrapTable(
					{
						idField : "id",
						uniqueId : "id",
						pagination : true,
						...
						pageSize : 5,
						pageList : [ 5, 10, 15 ],
						url : "/nutritionist/geTDietApplication",
						columns : [
								{
									field : "id",
									visible : false,
	...
	function accept(id,userId) {
        isAccept = window.confirm("确定同意其成为营养师?");
        if(isAccept==true){
        	$.ajax({
        		url:"/nutritionist/updatedietapplication",        		
        		async:false,
        		type:"post",
        		data:{
        			id:id,
        	...
        	success:function(data){
        		if(data.status==200){
					$("#nutritionisttable").bootstrapTable('remove', {
						field : 'id',
						values : [ parseInt(id) ]
					});
					alert(data.msg);
	...
	function refuse(id,userId){
		isRefuse = window.confirm("确定拒绝其成为营养师?");
		if(isRefuse==true){
			$.ajax({
				url : "/nutritionist/updatedietapplication",
				data:{
