									...
									},
									{
										...
										formatter : function operate(value,
											...
											return '<button type="button" class="btn btn-default btn-sm" onclick="replycomment(\''
													+ row.id
													+ '\',\''+row.commentcontent+'\')"> <span class="glyphicon glyphicon-share-alt"></span> 回复留言</button>&nbsp<button type="button" class="btn btn-default btn-sm" onclick="deletecomment(\''
													+ row.id
													+ '\')"><span class="glyphicon glyphicon-remove"></span> 删除留言</button>'
										}
									} ],
						})
		function replycomment(id,commentcontent) {
			$("#commentModal #commentid").val(id);
			$("#commentModal #commentcontent").val(commentcontent);						
			$("#commentModal").modal();					
		}
		function savereplycomment(){
			$.ajax({
				...
				data : {
					...
					replycontent : $("#commentModal #commentcontent").val(),					
