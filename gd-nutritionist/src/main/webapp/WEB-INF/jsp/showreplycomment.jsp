...
<body>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group" style="height:120px;">
						...
						<div class="form-control" id="replycontent" style="height:90%;overflow-y:auto;"></div>
					</div>
				</div>
	...
	<script type="text/javascript">
							...
							columns : [
									...
									},
									{
										...
										formatter : function operate(value,
												row, index) {																														
											return '<button type="button" class="btn btn-default btn-sm" onclick="replycomment(\''+row.id+'\')"><span class="glyphicon glyphicon-align-justify"></span> 留言详情</button>&nbsp<button type="button" class="btn btn-default btn-sm" onclick="deletecomment(\''+ row.id+ '\')"><span class="glyphicon glyphicon-remove"></span> 删除留言</button>';
										}
									} ],
						})
		...
		function replycomment(id) {			
			 $.ajax({
				 url:"/comment/getSingleReplycomment/"+id,
				 async:false,
				 success:function(data){
					 if(data.status==200){
						 $("#commentModal #commentcontent").html(data.data.commentcontent),
						 $("#commentModal #replycontent").html(data.data.replycontent), 
						 $("#commentModal").modal(); 
					 }
					 else
						 alert(data.msg);
